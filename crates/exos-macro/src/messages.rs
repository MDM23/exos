...
impl Declaration {
    /// The functions, one per message, written where the block was.
    ///
    /// No module around them: a message is called by its own name, and a crate
    /// that wants one puts the block inside a module of its choosing.
...
impl Message {
    ...
    /// follow it as a list. Where the function came from goes last, since
    /// nothing around it says so once the block expands into plain functions.
    fn documentation(&self) -> Vec<TokenStream> {
        ...
        docs.push(quote! { #[doc = ""] });
        docs.push(quote! {
            #[doc = "Generated by `exos::messages!`, and reads `exos::locale()`: \
                     outside a request, or inside a live fragment, it panics."]
        });
...
mod tests {
    ...
    fn a_message_becomes_a_function_that_reads_the_locale() {
        ...
        assert!(!expanded.contains("mod"), "{expanded}");
