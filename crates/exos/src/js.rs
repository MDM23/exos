...
/// Records a call to `url`, which is expected to be complete.
///
/// Generated by the method attributes, which build the URL through
/// [`url`](crate::url) so that it carries the application's base; there is no
/// need to call this directly. Nothing is prefixed here, so that one place
/// decides what a route's URL is rather than two.
///
/// # Panics
///
/// If called outside a handler; see [`emit`].
