...
/// One field of a model, handle and all.
///
/// The signal it wraps is the value, and it derefs to that, so everything a
/// signal does a field does: `form.email.get()`, `bind(&form.email)`,
/// `Effect::set(&form.email, ..)`. What it adds is what validation put beside
...
/// Declared exactly as the signal it wraps, so a handle in an attribute block
/// still puts its fields on the document.
impl<T> crate::IntoAttributes for &Bound<T> {
    fn write(self, attributes: &mut crate::Attributes) {
        crate::IntoAttributes::write(&self.signal, attributes);
    }
}
...
/// Written out rather than derived, because a derive puts `M: Copy` on the
/// impl and a model is never `Copy`. The token is a `&'static str` whatever `M`
/// is, so the bound would only ever have been wrong.
